...
It was never the plugin. It was the auth path, and it had been hiding in plain sight because nobody varies the thing they assume is inert. That is the whole post in one anecdote: **a single run is one sample from a noisy process, and the noise has structure that will fool you.**
...
The protocol is deterministic. The **outcome is not.** Run the identical protocol twice and you get a different wiki and a different set of query trajectories. One execution produces exactly one sample. That is the fact everything else follows from.
...
There is a punchline that makes the flood even more clearly a measurement artifact: **100% of `related:` edges duplicate a link the page body already makes.** The edge adds no graph connectivity the wiki did not already have. So the terrifying 434-versus-52 swing changed a *count* and changed nothing about what the wiki could actually answer. The scariest number in the investigation was inert.
...
## Confounds wear the costume of effects
