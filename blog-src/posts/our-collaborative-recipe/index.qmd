...
![The architecture: the non-proprietary template instantiates your agent (you plus an LLM, with tools, gates, and hooks) and its llm-wiki memory (an RDF and SPARQL graph), and both tether to a shared data fabric. Marketplaces and connectors feed up into the fabric, connectors reach out to external data such as PAD, and the federation makes the memory reachable by peers and shared bundles, read through an MCP server by hosts like Claude Desktop and Cursor.](architecture-diagram.png)
