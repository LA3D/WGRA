...
- **Pushing is collision-safe.** A push-with-retry protocol lets several people or agents push the same wiki without clobbering each other: append-only **index** and **log** files union-merge automatically, and a rare page conflict is left with markers for the agent to resolve on its next turn.
...
::: {.notes}
Plugin guidance: every wiki edit ends with a local commit in `.llm-wiki/` (run without asking; reversible), and push only when the user asks; the user just says "pull/push the wiki repo" and the agent runs the git. The collision-safe push is the ecosystem's Wiki Write Protocol: the `wiki_push` wrapper (scripts/wiki-write-protocol/protocol.sh), optimistic push-with-retry, CI-validated on 9 scenarios; `index_*`/`log_*` merge via `.gitattributes` `merge=union` so concurrent appends never conflict; content conflicts leave markers for the agent's next turn (the `llm_resolve` no-op defers to the agent). Accuracy note: this wrapper lives in the project/template tooling (`wiki/agents/wiki-write-protocol.md` + `scripts/wiki-write-protocol/`), not verified in the base llm-wiki plugin 0.4.0, so it is described here as the ecosystem's write protocol, not a base-plugin feature.
:::
...
::: {.notes}
Head-to-head source: the WGRA post "llm-wiki vs RAG" and Wiki-vs-RAG-First-Run-Results / Cross-Model-Wiki-Reading. n=40 on one real research project, blind judge; 90% vs RAG's 42%, a perfect 3.00 on method/judgment questions, zero fabrications; model-alone 2% shows the wiki adds the value. Scaling source: `llm-wiki-test-harness/docs/scaling-report.md` (Chris Sweet, 2026-09-08), real Sonnet 5 query runs (not projections) over 500 to 4,013 real arXiv pages around a 12-paper gold set; current one-line index; recall by gold-keyword match on specific-fact questions. Headline: recall flat (100% to 98%) and per-query cost bounded (3-6 reads, 5-7 greps) across 8x growth because the agent greps the index. The two panels are DIFFERENT experiments (blind-judge QA vs retrieval-recall) and are labeled as such; do not merge the numbers.
:::
...
::: {.notes}
The interactive graph of this project's own wiki, generated by the wiki-kg / wiki-graph-viz plugin we built this session. Drag nodes to explore, search by name, and color by type or by recency (git dates). Same structure a reader gets from the GitHub wiki; the typed edges are topology, not the query path.
:::
...
::: {.notes}
Verified against the marketplace repo: LA3D-LLM-Agents/llm-wiki-marketplace lists two plugins, wiki-kg (build/query/visualize the KG) and wiki-agent-msg (cross-harness messaging), with a shared plugin tree carrying both `.claude-plugin` and `.codex-plugin` manifests. Same marketplace-add / plugin-add pattern as the core plugin from llm-wiki-colab; swap the plugin name. This is where the graph on the previous slide came from. The Turtle snippet is a real node from the graph-full.ttl wiki-kg produced for this wiki (Constraining-the-Field-for-Trust: type Synthesis, extends Lightweight-Structural-Graphs, mentions LLM-Wiki-Read-Path/Vision); only the ontology prefix was renamed ns1: to kg: and the mentions list trimmed for legibility.
:::
