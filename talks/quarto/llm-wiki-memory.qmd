...
[Life expectancy vs GDP, OWID r6-1. The graph slide showed one hard chart; the same extractor handled other types too]{.kicker}

```{=html}
<img src="slide-assets/type2_dense-bubble.png" style="display:block;margin:0.35em auto 0.2em;max-width:100%;max-height:415px;border:1px solid #d8cfb6;border-radius:4px;">
```

*Left: the model's detected centers, erosion strips the country labels and a distance transform splits touching bubbles. Right: its pixel-check, one center per bubble and none on the colored labels.*

::: {.callout-highlight}
Density correlation 0.94, and 132 of 165 markers recovered.
:::

::: {.notes}
Source: figure-recovery-suite/docs/llm-wiki-deck-handoff/panels and its README, corpus owid-r6-1. Left is the model's own detection artifact, right is its verification overlay. These three appendix slides show the free-rein extractor generalizes past the el-94 combo chart on the earlier graphs slide. Panel titles were cropped off, they duplicated the heading and carried em dashes.
:::

## Also handled: multi-series time-series lines {.smaller}

[Annual CO2 emissions, OWID r6-1]{.kicker}

```{=html}
<img src="slide-assets/type3_multi-line.png" style="display:block;margin:0.35em auto 0.2em;max-width:100%;max-height:415px;border:1px solid #d8cfb6;border-radius:4px;">
```

*Left: where the model looked, attention concentrated on the fused low-emission band. Right: its pixel-check, all seven country series re-traced onto their own curves.*

::: {.callout-highlight}
All 7 series traced onto their lines, overlay-verified across the full record.
:::

::: {.notes}
Honest note to voice: on this chart the current per-type scorer reports curve F1 0.34 despite the verified trace, a measurement-layer gap (out-of-range points plus a tight tolerance), so the panel and the slide report the overlay-verified trace, not a score. The left attention map is a post-hoc reconstruction from saved zoom crops, a lower bound on where it looked, not live eye-tracking. Source: panels README, owid-r6-1.
:::

## Also handled: grouped bars with error bars {.smaller}

[Synthetic throughput benchmark, synthetic-r4-1]{.kicker}
